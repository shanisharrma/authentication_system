...
import { AuthMiddleware, RateLimit, ValidateRequestMiddleware } from '../../middlewares';
...
router.route('/register').post(RateLimit, ValidateRequestMiddleware.validateRequest(registerSchema), AuthController.register);
...
router.route('/login').post(RateLimit, ValidateRequestMiddleware.validateRequest(loginSchema), AuthController.login);
...
router.route('/forgot-password').post(RateLimit, ValidateRequestMiddleware.validateRequest(forgotPasswordSchema), AuthController.forgotPassword);

// Reset Password : PUT /api/v1/reset-password/:token
router.route('/reset-password/:token').put(RateLimit, ValidateRequestMiddleware.validateRequest(resetPasswordSchema), AuthController.resetPassword);
