...
(() => {
    try {
        ...
        if (connection) {
            Logger.info(Enums.EApplicationEvents.DATABASE_CONNECTED, { meta: {} }); // Log successful DB connection
        }

        // Initialize Rate Limiter
        initRateLimiter(connection); // Set up rate limiting
        Logger.info(Enums.EApplicationEvents.RATE_LIMITER_INITIATED); // Log rate limiter initialization

        // Log application startup details
        Logger.info(Enums.EApplicationEvents.APPLICATION_STARTED, { meta: { PORT: ServerConfig.PORT, URL: ServerConfig.SERVER_URL } });
    } catch (error) {
        Logger.error(Enums.EApplicationEvents.APPLICATION_CONNECTION_ERROR, { meta: error }); // Log connection errors

        // Gracefully shut down the server in case of error
        server.close((error) => {
            if (error) {
                Logger.error(Enums.EApplicationEvents.APPLICATION_CONNECTION_ERROR, { meta: error });
            }
